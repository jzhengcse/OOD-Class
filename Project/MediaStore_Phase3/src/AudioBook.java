...
//	private static final String mediaType="Audio Book"; //mediaType of audio book
	...
	public String toString() {
		String fs;
		fs=String.format("Name: %s  %nAuthor: %s  %nTime: %s  %nGenre: %s  %nAverage Ranking: %d  %nPrice: %.2f  %nNumberSold: %d  %nRating: %.2f", name,author,time,genre,rank,price,soldNum,rate);
//		fs=String.format("Media Type: Audio Book  Name: %s  Author: %s  Time: %s  Genre: %s  Average Ranking: %d  Price: %.2f  NumberSold: %d  Rating: %.2f", name,author,time,genre,rank,price,soldNum,rate);
		return fs;
